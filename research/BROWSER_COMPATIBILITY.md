...
| No background page or service worker | Very helpful on Firefox for Android, where background service workers have historically been a Manifest V3 compatibility gap. |
...
| Page code is normally injected only after the user opens and uses the popup | Fits the temporary permission model of `activeTab`. A separate **Remember on this site** switch can register the same local scripts for one hostname, without required access to all sites. |
...
4. Add optional hostname-scoped navigation continuity without a background worker or required host access. **Implemented.**
